...
function routeModal(r){r=r||{};modal((r.id?'Edit':'New')+' route',`
  ...
  <div class="field"><label>Type</label><select id="r_type">${['sparrow','hms','insoft','insoft2','insoftpanel','quickconnect','sociair','aakash','webzonesms','spellcpaas','routegod','spell','smpp','custom','globalzms','nestsms','nestpanel','nepal2rs','insoftsms','insoftsms2','insoftweb','arcbridge'].map(t=>`<option ${r.type===t?'selected':''}>${t}</option>`).join('')}</select></div></div>
